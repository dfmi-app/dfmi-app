#!/usr/bin/env node
/**
 * buyer-ollama.mjs — the same buyer agent on any OpenAI-compatible endpoint with tool calling.
 *
 ...
 *
 * The rules and tools come from buyer-core.mjs and are identical to buyer.mjs (Claude); the spending bounds are enforced in buykit and on-chain.
 * Only the model loop differs: send messages + tools → run any tool_calls locally → append results → repeat.
 *
 *   node buyer-ollama.mjs "Pay invoice inv_…: 0.020287 dUSD to 0x<seller> on eip155:112172."
 *   node buyer-ollama.mjs --repl
 */
import { createInterface } from 'node:readline';
import { readFileSync, existsSync } from 'node:fs';
import { SYSTEM, TOOLS, execTool } from './buyer-core.mjs';
...
const MAX_TURNS = Number(env('BUYER_MAX_TURNS', 12));
...
async function run(prompt) {
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    for (const call of m.tool_calls) {
      ...
      if (process.env.BUYER_DEBUG) console.error(`  ← ${content.slice(0, 300)}`);
...
if (argv[0] === '--repl') {
  ...
  const ask = () => rl.question('buyer> ', async (line) => { if (!line.trim()) return ask(); try { await run(line); } catch (e) { console.error('error:', e.message); } ask(); });
  ask();
} else if (argv.length) {
  await run(argv.join(' ')).catch(e => { console.error('error:', e.message); process.exit(1); });
} else {
  console.error('usage: node buyer-ollama.mjs "<instruction>"   |   node buyer-ollama.mjs --repl');
  process.exit(2);
}
